...
class StorageIoTests(unittest.TestCase):
	def test_load_vault_returns_none_when_path_does_not_exist(self):
		with tempfile.TemporaryDirectory() as tmpdir:
			...
			with patch("builtins.print"):
				self.assertIsNone(load_vault(str(path), VALID_MASTER_PASSWORD))
	...
	def test_load_vault_delegates_to_pwd_manager_for_existing_path(self):
		with tempfile.TemporaryDirectory() as tmpdir:
			...
			with patch("storage.io.PwdManager.from_encrypted_file", return_value=manager) as loader:
				self.assertIs(load_vault(str(path), VALID_MASTER_PASSWORD), manager)

		loader.assert_called_once_with(str(path), VALID_MASTER_PASSWORD)
	...
	def test_create_and_load_vault_touches_file_and_delegates(self):
		with tempfile.TemporaryDirectory() as tmpdir:
			...
			with patch("storage.io.PwdManager.pwd_manager_from_pwd", return_value=manager) as factory:
				self.assertIs(create_and_load_vault(str(path), VALID_MASTER_PASSWORD), manager)

			self.assertTrue(path.exists())

		factory.assert_called_once_with(str(path), VALID_MASTER_PASSWORD)
	...
	def test_create_and_load_vault_propagates_master_password_rejection(self):
		with tempfile.TemporaryDirectory() as tmpdir:
			path = Path(tmpdir) / "new.vault"

			with patch("storage.io.PwdManager.pwd_manager_from_pwd", side_effect=KeyError("bad password")):
				with self.assertRaisesRegex(KeyError, "bad password"):
					create_and_load_vault(str(path), "weak")
